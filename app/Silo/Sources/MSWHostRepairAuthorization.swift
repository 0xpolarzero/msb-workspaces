...
struct MSWHostRepairAuthorization: MSWHostRepairAuthorizing, Sendable {
    ...
    typealias Command = @Sendable (MSWCommand) async throws -> MSWCommandResult
    ...
    private static func rootRepairScript(records: [MSWWorkspaceNetworkRecord]) -> String {
        ...
        loop_script="$(/usr/bin/mktemp /tmp/msw-loopback.XXXXXX)"
        ...
        /usr/bin/awk '/^# BEGIN MSW WORKSPACES$|^# BEGIN SILO MANAGED HOSTS$/{skip=1;next}/^# END MSW WORKSPACES$|^# END SILO MANAGED HOSTS$/{skip=0;next}!skip{print}' /etc/hosts >"$hosts_tmp"
        ...
        daemon_plist="/Library/LaunchDaemons/dev.msw.loopback-aliases.plist"
        plist_tmp="$(/usr/bin/mktemp /tmp/msw-loopback.XXXXXX.plist)"
        /bin/cat >"$plist_tmp" <<'MSW_PLIST'

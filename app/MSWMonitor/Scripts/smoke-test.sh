...
case "${1:-}" in
  ...
  --failure-only)
    ...
    ;;
  *)
    print -u2 "usage: $0 [--monitor-only|--repair-only|--picker-only|--preferences-only|--navigation-only|--backup-only|--backup-result-only|--backup-operations-only|--network-only|--files-cache-only|--failure-only]"
    exit 64
    ;;
esac

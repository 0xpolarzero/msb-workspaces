...
describe("application", () => {
  it("opens on the nested sandbox Overview with compact navigation", () => {
    ...
    expect(sandboxList.closest('[data-slot="scroll-area"]')).toHaveClass("min-h-0", "flex-1")
  ...
  it("uses one global sandbox filter across Files, Logs, Network, and Activity", async () => {
    ...
    expect(within(repositoriesPane).getByRole("group", { name: "Repository pane controls" })).toContainElement(panel.getByRole("button", { name: "Collapse repositories" }))
    expect(within(fileTreePane).getByRole("group", { name: "File tree pane controls" })).toContainElement(panel.getByRole("button", { name: "Collapse file tree" }))
    ...
    expect(repositoriesPane.querySelector('[data-files-pane-scroll="repositories"]')).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
    expect(fileTreePane.querySelector('[data-files-pane-scroll="file-tree"]')).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
    ...
    expect(logs).toHaveClass("min-h-0", "flex-1", "overflow-hidden")
    expect(logs.querySelector('[data-table-scroll="logs"]')).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
    ...
    expect(activity).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
  ...
  it("shows one truthful network table and uses the selected browser for opening ports", async () => {
    ...
    expect(network).toHaveClass("min-h-0", "flex-1")
    expect(network.querySelector('[data-table-scroll="network"]')).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
